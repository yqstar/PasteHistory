...
一个常驻 macOS 菜单栏的剪贴板历史工具。原生 AppKit/Swift，单文件，无第三方依赖，本地存储、不联网。
...
- **菜单栏快捷访问**：点击菜单栏图标，下拉显示最近 30 条；点任意一条即写回剪贴板并自动粘贴（前 9 条有 ⌘1–⌘9 快捷键）
- **代码片段**：菜单栏「代码片段 ▸」子菜单、片段选择器（⌘⇧S）或片段自身的全局快捷键三种方式都能一键粘贴；片段数据存在 `~/Library/Application Support/PasteHistory/snippets.json`，可直接编辑该文件来增删改（编辑后重启 App 生效）
- **设置窗口**：一页式分组卡片表单，只管两个唤出快捷键、开机自启动、和快速打开两个选择器
...
- **本地持久化**：`~/Library/Application Support/PasteHistory/`，重启保留，上限 200 条（超出自动丢弃最旧的）
- **去重**：重复内容不新增，只移到最前
...
用的时候三选一：**片段选择器（默认 ⌘⇧S）搜索后回车**、**菜单栏图标 → 代码片段 ▸ → 选片段**、或直接按片段的全局快捷键——内容即写回剪贴板并自动粘贴（片段粘贴不会污染历史记录）。
...
- 仅记录通用剪贴板（`NSPasteboard.general`），每 0.5 秒轮询一次变化。
